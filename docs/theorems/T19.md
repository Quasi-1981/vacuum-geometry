...
??? note "Probe code (`S960.py`)"
    ```python
    --8<-- "src/probe/S960.py"
    ```

**Result:** asserts_passed=16 · FAIL=0

??? note "Run log (paraphrased under the word fence — see provenance) (`S960_run.log`)"
    ```text
    --8<-- "src/probe/S960_run.log"
    ```
